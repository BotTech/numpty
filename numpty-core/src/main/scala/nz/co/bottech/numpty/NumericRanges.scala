...
import nz.co.bottech.numpty.NumericRanges._
...
class NumericRanges[A] protected(private[numpty] val tree: Tree[A],
...
object NumericRanges {
  ...
  /**
    ...
    */
  private[numpty] def minimize[A: Ordering](ranges: Seq[NumericRange[A]]): List[NumericRange[A]] = {
  ...
  private[numpty] sealed trait Tree[+A] extends AbstractSeq[A] with IndexedSeq[A] with IndexedSeqLike[A, Tree[A]] {
    ...
    private[numpty] def balanced: Boolean
  }
  ...
  private[numpty] case class Node[A: Integral](range: NumericRange[A],
                                                 smaller: Tree[A],
    ...
    override private[numpty] val balanced: Boolean = smaller.length == larger.length && smaller.balanced && larger.balanced
  ...
  private[numpty] case object Empty extends Tree[Nothing] {
    ...
    override private[numpty] val balanced: Boolean = true
  ...
  protected class NumericRangeBuilder[B](implicit num: Integral[B]) extends mutable.Builder[B, NumericRanges[B]] {
    ...
    override def result(): NumericRanges[B] = {
      previous match {
        ...
        case Left(Nil)                            =>
        case Left(_)                              =>
          throw new IllegalStateException("Four or more elements had been stashed")
